...
from system.osi import run_command, create_tmp_dir, is_share_mounted, \
    is_mounted, get_dev_byid_name, convert_to_kib, toggle_path_rw, get_full_path
...
def get_pool_info(disk):
    ...
    """
    cmd = [BTRFS, 'fi', 'show', get_full_path(disk)]
...
def resize_pool(pool, dev_list_byid, add=True):
    ...
    """
    dev_list_byid = [get_full_path(d) for d in dev_list_byid]
...
def mount_root(pool):
    ...
    for device in pool.disk_set.all():
        mnt_device = (get_full_path(device.name))
        if (os.path.exists(mnt_device)):
            mnt_cmd = [MOUNT, mnt_device, root_pool_mnt, ]
            if (len(mnt_options) > 0):
                mnt_cmd.extend(['-o', mnt_options])
...
def mount_share(share, mnt_pt):
    ...
    pool_device = (get_full_path(share.pool.disk_set.first().name))
...
def rollback_snap(snap_name, sname, subvol_name, pool):
    ...
    dpath = get_full_path(pool.disk_set.first().name)
...
def btrfs_uuid(disk):
    ...
    o, e, rc = run_command(
        [BTRFS, 'filesystem', 'show', get_full_path(disk)])
    return o[0].split()[3]
