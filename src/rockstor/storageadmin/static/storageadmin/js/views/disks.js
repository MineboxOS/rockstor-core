...
DisksView = RockstorLayoutView.extend({
    ...
    initHandlebarHelpers: function () {
        ...
        // Identify Minebox lower-level disks by return of true / false.
        // Works by examining the Disk.role field. Based on sister handlebars
        // helper 'isRootDevice'
        Handlebars.registerHelper('isMBLower', function (role) {
            var roleAsJson = asJSON(role);
            if (roleAsJson == false) return false;
            // We have a json string ie non legacy role info so we can examine:
            if (roleAsJson.hasOwnProperty('mblower')) {
                // We have a  Minebox lower-level disk which can't be added to
        ...
        // Identify Minebox key US sticks by return of true / false.
        // Works by examining the Disk.role field. Based on sister handlebars
        // helper 'isRootDevice'
        Handlebars.registerHelper('isMBKey', function (role) {
            var roleAsJson = asJSON(role);
            if (roleAsJson == false) return false;
            // We have a json string ie non legacy role info so we can examine:
            if (roleAsJson.hasOwnProperty('mbkey')) {
                // We have a  Minebox key USB stick which can't be added to
                // pools or reformatted.
                return true;
            }
            // In all other cases return false.
